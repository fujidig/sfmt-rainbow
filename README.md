...
各step∈{417, 477, 1012, 1132}ごとに約50MBのテーブルを使います。
仕組み上100%初期seedが見つかるわけではないですが、90%ぐらいは成功します。
検索は4秒(並列化したもの) or 26秒(without 並列化)程度で終わります。
...
で417.binというテーブルを作成します (M1 Macだと2時間かかりました)。
